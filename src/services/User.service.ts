...
import { Role } from '@/admin/interfaces/role.interface';
...
export default class User {
  ...
  public async getRoles() {
    const result = (
      await db
        .select({ ...userColumns, profile: profileColumns })
        .from(userTable)
        .innerJoin(profileTable, eq(profileTable.userId, userTable.id))
        .where(eq(userTable.id, this._userId))
    )[0] as UserType & { roles: Omit<Role, 'permissions'>[] };

    result.roles = await db
